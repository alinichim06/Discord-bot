...
async def play(ctx,song):
    if not ctx.message.author.voice:
        await ctx.send("{} [-] Please join in a voice channel".format(ctx.message.author.name))
        return
    else:
        ...
        try :
            ...
            if song in song_list:
                ...
                await ctx.send('[info] Playing: ' + song)
            else :
                await ctx.send("[-] This song is not registered!")
        except :
            await ctx.send("[info] No song is playing right now!")

@bot.command()
async def list(ctx):
    await ctx.send("[info] Songs:")
...
async def scram(ctx):
    ...
    if voice_client.is_connected():
        ...
        await ctx.send("[info] Bot is not connected to any channel")
...
async def pause(ctx):
    ...
    if voice.is_playing():
        voice.pause()
        await ctx.send("[info] Song paused")
    else:
        await ctx.send("[info] No song is playing right now!")
...
async def resume(ctx):
    ...
    if voice.is_paused():
        ...
        await ctx.send("[info] Song is already playing")
